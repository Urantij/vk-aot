...
public class GroupJsonConverter : Newtonsoft.Json.JsonConverter
{
	...
	public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
	{
		...
		var group = new Group
		{
			...
			MainSection = !response.ContainsKey("main_section") ? MainSection.NoSection : Utilities.Deserialize<MainSection>(response["main_section"].ToString()),
			...
			AgeLimits = !response.ContainsKey("age_limits") ? AgeLimit.Withoutlimit : Utilities.Deserialize<AgeLimit>(response["age_limits"].ToString()),
			MemberStatus = !response.ContainsKey("member_status") ? MemberStatus.Invited : Utilities.Deserialize<MemberStatus>(response["member_status"].ToString()),
			PublicDateLabel = response["public_date_label"],
			Wall = !response.ContainsKey("wall") ? WallType.Off : Utilities.Deserialize<WallType>(response["wall"].ToString()),
		};
